...
from trl.trainer.grpo_trainer import (Any, AutoModelForCausalLM, AutoModelForSequenceClassification, AutoTokenizer, Dataset, GRPOConfig, GRPOTrainer, GenerationConfig, IterableDataset, Optional, PeftConfig, PreTrainedModel, PreTrainedTokenizerBase, RepeatRandomSampler, RewardFunc, Sampler, SyncRefModelCallback, Trainer, TrainerCallback, Union, VLLMClient, apply_chat_template, broadcast_object_list, create_reference_model, deepspeed, defaultdict, gather, gather_object, generate_model_card, get_comet_experiment_url, is_conversational, is_deepspeed_zero3_enabled, is_peft_available, is_peft_model, is_rich_available, is_vllm_available, is_wandb_available, maybe_apply_chat_template, nn, nullcontext, os, pad, prepare_deepspeed, print_prompt_completions_sample, profiling_context, profiling_decorator, set_seed, textwrap, torch, transformers, unwrap_model_for_generation, version, wandb, warnings, os, torch, transformers, GRPOTrainer, Trainer, gather, os, torch)
...
class UnslothGRPOTrainer(_UnslothGRPOTrainer):
    ...
    def __init__(
        ...
        peft_config = None,
        opt=None,
        data_filter_fn: Optional[Callable] = None,
        **kwargs
    ):
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        # 如果定义了 data_filter_fn，则对数据集进行筛选
        if self.data_filter_fn is not None and self.state.global_step % self.args.logging_steps == 0:
            if not hasattr(self, "original_dataset"):
                self.original_dataset = self.train_dataset

            # 从 original_dataset 中筛选样本
            self.train_dataset = self.data_filter_fn(self.original_dataset, self.opt)
            print("Filtered once")

        # 调用父类的 _prepare_inputs 方法
